...
public class SCP096BlockBreakingGoal extends Goal {
	...
	private void fillTargetBlocks() {
		...
		for (int i = 0; i < mobHeight; i++) {
			BlockHitResult rayTraceResult = this.scp096.getWorld().raycast(new RaycastContext(this.scp096.getPos().add(0, i + 0.5d, 0), Objects.requireNonNull(this.target).getEyePos().add(0, i, 0), RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, this.scp096));
	...
	private float getDigSpeed() {
		...
		if (this.scp096.hasStatusEffect(StatusEffects.MINING_FATIGUE)) {
			float miningFatigueAmplifier = switch (Objects.requireNonNull(this.scp096.getStatusEffect(StatusEffects.MINING_FATIGUE)).getAmplifier()) {
				case 0 -> 0.3F;
				case 1 -> 0.09F;
				case 2 -> 0.0027F;
				default -> 8.1E-4F;
