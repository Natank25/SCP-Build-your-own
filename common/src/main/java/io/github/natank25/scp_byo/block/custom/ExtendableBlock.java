...
public class ExtendableBlock extends Block {
	...
	public ExtendableBlock(Settings settings, int maxHigh) {
		super(settings);
	...
	private void isSameAsBlockBelow(BlockState state, World world, BlockPos pos) {
		if (world.getBlockState(pos.down()).getBlock() == state.getBlock())
			...
			world.setBlockState(pos, world.getBlockState(pos.down()).getBlock() == ModBlocks.SLIDING_DOOR.get() ? state.with(POS, 1).with(PLACE, BlockPlace.UPPER) : state.with(POS, 1).with(PLACE, BlockPlace.LOWER), 3);
	}
}
